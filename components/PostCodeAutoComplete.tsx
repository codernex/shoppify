...
export default function PostCodeAutoComplete({countryData,form,countryState}: PostCodeData) {
    ...
    return (
        <Combobox value={selected} onChange={(value)=>{
            setSelected(value)
            const state= countryState.find(s=>s.shortCode===value.subdivisionCode)
            ...
            >
                <Combobox.Options
                    className="absolute mt-1 max-h-60 w-full max-w-[350px] overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                    {filteredPost.length === 0 && query !== '' ? (
                        ...
                        >
                          <span>{post.postCode}</span>
                            ...
                            <span>, {
                                countryState.find((item)=>item.shortCode===post.subdivisionCode)?.name
                            }</span>
                        </span>
                                        {selected ? (
                                            <span
