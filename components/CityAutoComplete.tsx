...
export default function CityAutoComplete({countryData,form,countryState}: ICityProps) {
    ...
    return (
        <Combobox value={selected} onChange={(value)=>{
            ...
            >
                <Combobox.Options
                    className="absolute mt-1 max-h-60 w-full max-w-[380px] overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                    {filteredCityData.length === 0 && query !== '' ? (
                        ...
                        >
                          <span>{city.postCode}</span>
                            ...
                            <span>, {
                                countryState.find((state)=>state.shortCode===city.subdivisionCode)?.name
                            }</span>
                        </span>
                                        {selected ? (
                                            <span
